...
uses
  Classes, SysUtils, dynlibs, Utils,
  //uDetectStr,
  uWlxPrototypes, WlxPlugin
  //DCClassesUtf8,
  //uDCUtils,
  //LCLProc, LCLType
  //, DCXmlConfig
...
begin
  ...
  Result := False;//FParser.TestFileResult(AFileName);
end;
...
begin
  ...
  if Assigned(ListGetPreviewBitmapW) then
    ...
    Result := ListGetPreviewBitmap(PAnsiChar(UniToSys(FileToLoad)), Width, Height,
      PChar(contentbuf), length(contentbuf));
end;

function TWlxModule.CallListGetPreviewBitmapFile(FileToLoad,OutputPath: String; Width,
  Height: Integer; contentbuf: String): string;
begin
  Result := '';
  if Assigned(ListGetPreviewBitmapFileW) then
    Result := ListGetPreviewBitmapFileW(PWideChar(UTF8Decode(FileToLoad)),PWideChar(UTF8Decode(OutputPath)), Width, Height,
      PChar(contentbuf), length(contentbuf))
